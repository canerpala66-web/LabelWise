...
void main() {
  test('V5 wins over stale fallback score when current product data is available', () {
    ...
    expect(LabelWiseScoreService.scoreVersion, 'v5');
    expect(result.score, isNot(92));
    expect(result.score, lessThanOrEqualTo(60));
  });

  test('fallback score is used only when V5 cannot calculate at all', () {
    final result = LabelWiseScoreService.instance.calculate(
      const Product(
